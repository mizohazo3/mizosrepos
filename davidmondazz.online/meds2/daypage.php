        ...
        while ($row = $result->fetch_assoc()) {
          ...
          if(strtotime($dose_date) <= strtotime($side_date)){
              ...
              if (strtotime($date_after_5_half_lives) >= strtotime($side_date)) {
                ...
                echo '<a href="possible_sides.php?name='.$firstWord.'" style="text-decoration: none;color: inherit;">'.$row["medname"] .'</a> = '.$timeDiff. " hrs (Half: ".floatval($row["default_half_life"]).") (Remain: ".round($remainingAmount,2)." mg)<br>"; // Output medication name
