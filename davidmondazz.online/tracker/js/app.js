...
function startClientSideUpdates() {
    ...
    setInterval(() => {
        // Target both main cards and running items efficiently
        Object.keys(localTimers).forEach(timerIdStr => {
            const timerId = parseInt(timerIdStr);
            if (!localTimers[timerId]) return; // Should not happen with Object.keys, but safety first

            // Calculate current elapsed time based on locally stored start time and base
            const elapsed = localTimers[timerId].baseElapsed +
                (Date.now() - localTimers[timerId].startTime) / 1000;

            // Format the time directly, don't use the formatElapsedTimeClient function
            ...
            if (timerCard) {
                const currentTimeElement = timerCard.querySelector('.timer-current');
                const msElement = timerCard.querySelector('.timer-ms');

                if (currentTimeElement && msElement) {
                    // Update main time (HH:MM:SS) text node
                    if (currentTimeElement.childNodes.length > 0 && currentTimeElement.childNodes[0].nodeType === Node.TEXT_NODE) {
                        currentTimeElement.childNodes[0].nodeValue = formattedTime;
                    } else {
                         console.warn(`Unexpected structure for timer-current in timer ID ${timerId}.`);
                    }
                    // Update milliseconds span
            ...
            if (runningItem) {
                ...
                if (durationSpan) {
                     // Update duration span with the same format
...
/**
 * Render timers in the container
 */
function renderTimers() {
    // Clear the container
    timersContainer.innerHTML = '';
    
    let filteredTimers = [...timers]; // Create a copy to avoid modifying the original array
    
    // Apply different filtering based on whether we're searching or not
    if (currentSearchTerm) {
        // When searching, we're already getting filtered results from the API
        // No additional filtering needed, as search_timers.php returns all matching timers
        filteredTimers = timers;
    } else {
        // If not searching, show running, paused and sticky timers
    ...
    // Show message if no timers after filtering
    ...
    // Sort timers differently based on search context
    const sortedTimers = filteredTimers.sort((a, b) => {
        if (currentSearchTerm) {
            // For search results, just sort by total time (highest first)
            return b.total_time - a.total_time;
        } else {
            // Normal view: sticky first, then status, then total time
            // First sort by sticky status (sticky timers first)
            if (a.is_sticky && !b.is_sticky) return -1;
            if (!a.is_sticky && b.is_sticky) return 1;
            
            // Then sort by status (running & paused before idle)
            if (a.status !== 'idle' && b.status === 'idle') return -1;
            if (a.status === 'idle' && b.status !== 'idle') return 1;
            
            // Finally sort by total time (highest first)
            return b.total_time - a.total_time;
        }
    });
    
    // Render all filtered timers
    sortedTimers.forEach(timer => {
        renderSingleTimer(timer);
    });
}

/**
 * Render a message when there are no timers to display
 * @param {string} [message="No timers found"] - Optional custom message.
 * @param {string} [subMessage="Click \"Add Timer\" in the navigation bar to create a new timer."] - Optional custom sub-message.
 */
function renderNoTimersMessage(message = "No timers found", subMessage = 'Click "Add Timer" in the navigation bar to create a new timer.') {
    timersContainer.innerHTML = `
        <div class="col-12 no-timers">
            <i class="fas fa-clock fa-3x mb-3"></i>
            <h3>${message}</h3>
            <p>${subMessage}</p>
            ${message.includes("Add one?") ? 
                `<button class="btn btn-primary mt-3" data-bs-toggle="modal" data-bs-target="#addTimerModal">
                    <i class="fas fa-plus-circle me-2"></i>Add Timer
                 </button>` : ''
            }
        </div>
    `;
...
function renderSingleTimer(timer, insertBeforeElement = null, parentContainer = null) {
    // Check if element already exists to prevent duplicates during rapid updates
    if (document.getElementById(`timer-${timer.id}`)) {
        return;
    }
    const timerElement = document.createElement('div');
    timerElement.className = 'col-12'; // Changed from 'col-md-6 col-lg-4' to full width
    timerElement.id = `timer-${timer.id}`;
    timerElement.setAttribute('data-timer-id', timer.id);
    timerElement.setAttribute('data-status', timer.status);
    
    timerElement.innerHTML = createTimerHtml(timer);
    
    // Use parent container if provided, otherwise use main timersContainer
    const container = parentContainer || timersContainer;
    
    // Append timer to maintain position order (instead of prepend)
    if (insertBeforeElement) {
        container.insertBefore(timerElement, insertBeforeElement);
    } else {
...
function createTimerHtml(timer) {
    ...
    const stickyIcon = timer.is_sticky ? 'thumbtack' : 'thumbtack';
    ...
    // Remove level info section
    
    return `
        <div class="horizontal-timer-card ${timer.is_sticky ? 'sticky-timer' : ''}">
            ${stickyButton}
            <div class="timer-left">
                <div class="timer-title"><a href="timer_details.php?id=${timer.id}" style="text-decoration: none; color: inherit;">${timer.name}</a></div>
                <div class="timer-hours"><i class="fas fa-clock"></i> <strong>${formattedTotalHours}h</strong></div>
            </div>
...
function attachTimerEventListeners(timerElement, timer) {
    ...
    if (stickyBtn) {
        ...
        newStickyBtn.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            // Get the current data-sticky attribute value at click time
 ...
 * @param {number} timerId - Timer ID
 * @param {number} sticky - New sticky status (0 or 1)
 */
function toggleStickyTimer(timerId, sticky) {
    ...
    // Convert sticky to boolean for cleaner code
    const makeSticky = sticky === "1" || sticky === 1;
