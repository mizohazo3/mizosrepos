...
if (isset($_POST['addnew']) && $_POST['addnew'] == 'AddNew!') {
    ...
    if (empty($_POST['activity_name'])) {
        ...
        $insert->execute([$activity_name, $timestamp, $cats_list, 'off', '', $linkUrl]);
...
<div class="cats" style="border:2px solid #a9a9a9;border-radius:20px;margin:5px;"> <a href="../index.php" class="btn btn-secondary btn-sm" style="margin:5px;">Main</a> <a href="categories.php" class="btn btn-light btn-sm" style="margin:5px;">Categories</a> <a href="../timeline" style="margin:5px;"><img src="img/timeline_icon.png"></a><a href="../leave.php" class="btn btn-warning btn-sm" style="margin:5px;">Leave!</a></div> 
 <a href="index.php"><img src="img/icon.png" class="spin"></a><form action="index.php" method="post" style="display:inline">
...
if (isset($_POST['linkchange']) && $_POST['linkchange'] == 'Change!') {
    ...
    $linkmsg = '<img src="img/rightsign.png"> <b>' . $row['name'] . '</b> Updated link successfully!';
}
...
if (isset($_POST['editchange']) && $_POST['editchange'] == 'Edit!') {
    ...
    $linkmsg = '<img src="img/rightsign.png"> <b>' . $row['name'] . '</b> Edited to <b>' . $edit . '</b> successfully!';
}
...
if (isset($_GET['manage'])) {
    ...
    <img src="img/search.png" style="padding-bottom:5px;"> <input type="text" name="searchKey" value="' . $keyword . '" style="height:40px;font-size:20px;text-align:center;">
        ...
        if (!empty($_GET['searchKey'])) {
            ...
            if ($item['statusOn'] == 'on') {
               ...
               <input type="submit" name="stop" value="Stop" class="btn btn-danger btn-sm">
                ...
                $activeIcon = '<img src="img/on.png">';
                $fastStart = '';
            } else {
                $stopButton = '';
                $activeIcon = '';
                $fastStart = '<span name="' . $item['name'] . '" id="' . $item['id'] . '""><button><img src="img/starticon22.png" class="FastStart" style="opacity:0.6;border:2px solid #A7A7A7;border-radius:10px;padding:3px;"></button></span>';
            }
            ...
            if (!isset($_GET['manage']) and !isset($_GET['searchKey'])) {
                ...
                if ($item['statusOn'] == 'on') {
                   ...
                   <input type="submit" name="stop" value="Stop" class="btn btn-danger btn-sm" style="opacity:0.9;border-radius:12px;">
                    ...
                    $startButton = '<span name="' . $item['name'] . '" id="' . $item['id'] . '""><button><img src="img/starticon22.png" class="FastStart" style="opacity:0.6;border:2px solid #A7A7A7;border-radius:10px;padding:3px;"></button></span>';
                }
...
updateRealtime();
 ...
 $(document).on('click', "#StopAll",function(){
        ...
        if(confirm('Do you want to stop '+ name +' ?'))
        {
            $.ajax({
               ...
               success: function(data) {
                  // Instead of reloading, remove the stopped activities from UI
                  $(".multiplework").fadeOut(500, function() {
                    $(this).remove();
                  });
                  // Update the count in "Current Working" text
                  $(".currentWorking b").text("Current Working: (0)");
               }
            });
        }
   ...
   $(document).delegate(".FastStart", "click", function(){
    ...
    // Call the AJAX request without showing the confirmation
    $.ajax({
        type: 'GET',
        url: 'faststart.php',
        data: { name: name, id: id },
        ...
        success: function (response) {
            // Instead of reloading, get the HTML for the "Working" section
            $.ajax({
                type: 'GET',
                url: 'get_working_activity.php',
                data: { id: id },
                success: function(workingHtml) {
                    // Check if we already have a working section
                    if ($('.currentWorking').length) {
                        // Update the count in the "Current Working" section
                        var currentCount = parseInt($('.currentWorking b').text().match(/\d+/)[0]);
                        $('.currentWorking b').text('Current Working: (' + (currentCount + 1) + ')');
                        
                        // If we have a multiplework div, add to it, otherwise create one
                        if ($('.multiplework').length) {
                            $('.multiplework').append('<p style="display:inline;"><span class="line-break"></span></p>' + workingHtml);
                        } else {
                            $('.currentWorking div:first').append('<div class="multiplework">' + workingHtml + '</div>');
                        }
                    } else {
                        // Create a new working section from scratch at the top of the list div
                        var newSection = '<span class="currentWorking">' +
                            '<div style="border:2px black solid;border-radius: 1em;background: #ddd;padding: 15px;border: 2px solid white;">' +
                            '<div style="margin-bottom:10px;margin-left:15px;color:#1650DF;"><b>Current Working: (1)</b></div>' +
                            '<div style="border:2px solid;margin:5px;padding:14px;border-radius:10px;display: inline-block;">' +
                            workingHtml +
                            '</div></div><br><br></span>';
                        $('.list').prepend(newSection);
                    }
                    
                    // Hide the start button for this activity
                    $('span#' + id).parent().fadeOut(500, function() {
                        $(this).html('');
                    });
 ...
 $(document).delegate(".FastStop", "click", function(){
   ...
   $.ajax({
    type: 'GET',
    url: 'stop.php',
    data: { name: name, id: id, notes: "" },
    beforeSend: function () {},
    success: function (response) {
        // Remove this activity from the working section
        $(this).closest('.multiplework div').fadeOut(500, function() {
            $(this).remove();
            ...
            if (currentCount > 1) {
                ...
                // If this was the last activity, remove the entire working section
                $('.currentWorking').fadeOut(500, function() {
                    $(this).remove();
                });
            }
        });
        
        // Show the start button for this activity again
        $.ajax({
            type: 'GET',
            url: 'get_start_button.php',
            data: { id: id },
            success: function(buttonHtml) {
                $('span#' + id).html(buttonHtml);
                $('span#' + id).parent().fadeIn(500);
            }
        });
    }
});
...
function startTimer() {
  // Get all timer elements with class timerLive
  var timerElements = $('.timerLive');
  ...
  timerElements.each(function() {
    ...
    // Update immediately
    updateSingleTimer(timerElement, startTime);
...
function updateSingleTimer(element, startTime) {
  $.ajax({
    url: 'timerLive.php',
    type: 'GET',
    data: { id: startTime },
    success: function(timeDisplay) {
      element.html(timeDisplay);
    }
  });
}

    document.getElementById('select-all') && (document.getElementById('select-all').onclick = function() {
    ...
    document.getElementById('select-active') && (document.getElementById('select-active').onclick = function() {
    ...
    document.getElementById('select-locked') && (document.getElementById('select-locked').onclick = function() {
    ...
    document.getElementById('select-special') && (document.getElementById('select-special').onclick = function() {
...
$(document).ready(function() {
    // Initialize timers for any existing timer elements when page loads
    startTimer();
});
</script>
</body>
