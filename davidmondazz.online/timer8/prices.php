...
<script>
    ...
    function initSaveButtons() {
        // Initialize dose price save buttons
        ...
        document.querySelectorAll('.save-med-btn').forEach(button => {
            button.addEventListener('click', function() {
                // Get parent elements and inputs
                const medCard = this.closest('.form-row').closest('.med-card');
                const parentCol = this.closest('.col-md-6');
                const medId = this.getAttribute('data-med-id');
                const mgInput = parentCol.querySelector('input[name^="mg_"]');
                const doseInput = parentCol.querySelector('input[name^="dose_"]');
                const priceInput = parentCol.querySelector('input[name^="price_"]');
                ...
                // Validate inputs
                if (isNaN(mg) || mg <= 0 || isNaN(dose) || dose <= 0 || isNaN(price) || price <= 0) {
                    saveStatus.textContent = 'Error: All fields must be filled with valid numbers';
