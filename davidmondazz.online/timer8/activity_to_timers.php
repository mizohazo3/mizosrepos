...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["sqlfile"])) {
    ...
    try {
        ...
        $sql_insert = "INSERT INTO {$dest_table} (
                            name, accumulated_seconds, created_at,
                            is_running, current_level, notified_level, is_pinned
                        ) VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        $insert_name = ''; $insert_accumulated_seconds = 0; $insert_created_at = '';
        $insert_is_running = 0; $insert_current_level = 1; $insert_notified_level = 1; $insert_is_pinned = 0;

        $stmt->bind_param("sisiiii", // s=string, i=integer
            $insert_name, $insert_accumulated_seconds, $insert_created_at,
            $insert_is_running, $insert_current_level, $insert_notified_level, $insert_is_pinned
        );
...
function processInsertStatement(string $insert_sql, ?array $column_map, mysqli_stmt $stmt, int $line_number, int &$imported_count, int &$error_count): void
{
    ...
    global $insert_name, $insert_accumulated_seconds, $insert_created_at;
    global $insert_is_running, $insert_current_level, $insert_notified_level, $insert_is_pinned;
    ...
    foreach ($row_values_sets as $row_set_index => $row_set) {
        ...
        try {
            ...
            $status_idx = $column_map['status'] ?? null;

            // Check required columns exist in the map
            if ($name_idx === null || $time_spent_idx === null || $added_date_idx === null) {
                $error_count++; continue;
            }
            ...
            $raw_status = isset($values[$status_idx]) ? $values[$status_idx] : null;

            // --- Assign data and perform transformations using parseSqlValue ---
            $insert_name = parseSqlValue($raw_name);
            $insert_accumulated_seconds = timeToSeconds(parseSqlValue($raw_time_spent));
            $insert_created_at = formatDateForMySQL(parseSqlValue($raw_added_date));

            // Map 'status' from old table to 'is_running' (Assuming '1' means running, adjust if needed)
            $parsed_status = parseSqlValue($raw_status);
            $insert_is_running = ($parsed_status === '1' || $parsed_status === 1) ? 1 : 0;

            // Set defaults for new columns not in the source file
            $insert_current_level = 1; // Reset level on import? Or try to calculate? Defaulting to 1.
            $insert_notified_level = 1;
            $insert_is_pinned = 0; // Default pinned to false

            // --- Post-parsing Validation ---
            if ($insert_name === null || $insert_name === '') {
                $error_count++; continue;
            }
            if ($insert_created_at === null || trim($insert_created_at) === '') {
                $error_count++; continue;
            }
