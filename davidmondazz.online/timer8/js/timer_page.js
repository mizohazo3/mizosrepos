document.addEventListener('DOMContentLoaded', () => {
    // Check if we're on the search page (flag set in search.php)
    const isSearchPage = window.isSearchPage === true;
    
    // --- Configuration ---
    // --- Configuration (Timer Page Specific) ---
    const POLLING_INTERVAL_MS = 5000;
    const PROGRESS_UPDATE_MS = 1000; // Only update progress every second instead of every UI tick
    ...
    // Add a timestamp tracker for each timer's progress updates
    let progressUpdateTimestamps = {};
    ...
    const totalGainedValueEl = document.getElementById('total-gained-value');
    
    // Check if we're on a page that uses timers
    const hasTimerFeatures = !!timerList && !!timerTemplate && !isSearchPage;
    ...
    function calculateAndRenderProgress(timerEl, timerData, currentTotalHours, forceUpdate = false) {
        if (!timerEl || !timerData || !levelsConfig) return;

        // Implement debouncing for progress updates to prevent flickering
        const timerId = timerData.id;
        const currentTime = Date.now();
        
        // Skip updates unless forced or enough time has passed since last update
        if (!forceUpdate && progressUpdateTimestamps[timerId] && 
            (currentTime - progressUpdateTimestamps[timerId] < PROGRESS_UPDATE_MS)) {
            return;
        }
        
        // Update timestamp
        progressUpdateTimestamps[timerId] = currentTime;
        ...
        try {
            ...
            if (isMaxLevel) {
                ...
                if (nextThreshold === null || nextThreshold <= currentThreshold) {
                    ...
                    // Fix for NaN% issue - ensure we never divide by zero or undefined
                    progressPercentage = (!hoursNeeded || hoursNeeded <= 0) ? 
                        100 : // If hoursNeeded is zero or negative, set to 100%
                        Math.min(100, (hoursTowardsNext / hoursNeeded) * 100);
            ...
            if (progressText) {
                // Check if this is a "New Tracker By AI" timer by name
                const isAITracker = timerData.name && timerData.name.toLowerCase().includes('tracker by ai');
                
                // Always display percentage for AI trackers and generally prefer percentage over MAX
                // Only show MAX if it's truly at max level AND not an AI tracker
                const textContent = isNaN(progressPercentage) ? 
                    `${Math.floor(0)}%` : // If it's NaN, show 0% instead of MAX
                    (isMaxLevel && !isAITracker) ? 'MAX' : `${Math.floor(progressPercentage)}%`;
                
                // Only update text if it's actually changed
                if (progressText.textContent !== textContent) {
                    progressText.textContent = textContent;
                }
            }
        } catch (error) {
            // Silent error handling was here, but now let's at least set something sensible
            const progressText = timerEl.querySelector('.timer-progress-text');
    ...
    // Function to calculate and update the total gained from running timers
    function updateTotalGained() {
        if (!totalGainedValueEl || !timers || !levelsConfig) return;
        
        let totalGained = 0;
        
        // Loop through all timers and calculate earnings for running ones
        Object.values(timers).forEach(timer => {
            if (timer.is_running) {
                // Use the existing calculation function for current earnings
                const currentEarnings = calculateCurrentEarnings(timer, calculateCurrentSeconds(timer));
                totalGained += currentEarnings;
            }
        });
        
        // Update the total gained display
        totalGainedValueEl.textContent = `$${totalGained.toFixed(2)}`;
        
        // Find the indicator container
        const earningsIndicator = document.getElementById('current-earnings-indicator');
        if (earningsIndicator) {
            // Add active class when there are running timers
            if (totalGained > 0) {
                earningsIndicator.classList.add('active');
            } else {
                earningsIndicator.classList.remove('active');
            }
        }
    }
    ...
    function updateUITick() {
        ...
        for (const timerId in timers) {
            ...
            // Only update running timers' displays
            if (timerData.is_running) {
                ...
                try {
                    ...
                    const milliseconds = timerEl.querySelector('.milliseconds');
                    ...
                    // Update session earnings ($$) if the UI element exists
                    ...
                    if (sessionTotalEl) {
                        ...
                        sessionTotalEl.textContent = `$${currentEarnings.toFixed(2)}`;
                    }

                    // Calculate total hours consistently
                    const accumulatedHours = parseFloat(timerData.accumulated_seconds || 0) / 3600.0;
                    const sessionHours = calculateCurrentSessionHours(timerData);
                    const totalHours = accumulatedHours + sessionHours;
                    
                    // Update progress bar but don't force updates on every tick
                    calculateAndRenderProgress(timerEl, timerData, totalHours, false);
                } catch (e) {
                    console.error(`Error updating timer ${timerId} UI:`, e);
                }
            }
        }
        
        // Update the total gained from running timers
        updateTotalGained();

        // Update the connection status indicator if it exists
        if (connectionStatusEl) {
            connectionStatusEl.textContent = atLeastOneRunning ? 'Status: Active' : 'Status: Connected';
            connectionStatusEl.style.color = atLeastOneRunning ? 'var(--accent-success)' : 'var(--accent-secondary)';
        }
    }
    ...
    function initializeUI() {
        ...
        // Initialize search if we're not on the search page
        if (searchInput && !isSearchPage) {
            searchInput.addEventListener('input', filterTimers);
            searchInput.addEventListener('click', filterTimers);
        }
        
        // Initialize stop all button
        if (stopAllBtn) {
            stopAllBtn.addEventListener('click', stopAllTimers);
        }
        
        // Initialize reset all button if it exists
        if (resetAllBtn) {
            resetAllBtn.addEventListener('click', resetAllTimers);
        }
        
        // Fetch initial data and start polling if we have timer features and are not on search page
        if (hasTimerFeatures) {
            fetchInitialData()
                .then(() => {
                    startPolling();
        ...
        // Even if not on timer page, update running timer counter
