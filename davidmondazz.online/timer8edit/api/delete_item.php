...
try {
        ...
        if ($image_url) {
            ...
            if ($filePath && $allowedDir && strpos($filePath, $allowedDir) === 0 && file_exists($filePath)) {
                if (unlink($filePath)) {
                    ...
                    log_delete("WARNING: Failed to delete image file: $filePath", $log_file);
    ...
    $pdo->rollBack();
    log_delete("PDOException during delete: {$e->getMessage()} | SQL State: {$e->getCode()}", $log_file);
    error_log("Delete Item PDO Error: " . $e->getMessage());
    ...
    log_delete("General Exception during delete: {$e->getMessage()}", $log_file);
