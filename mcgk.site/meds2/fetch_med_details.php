...
try {
    // Prepare the query to fetch data from the medtrack table
    $query = "SELECT * FROM medtrack ORDER BY id DESC LIMIT 1";
    ...
    if ($fetch && !empty($fetch['medname'])) {
        ...
        $firstWord = isset($fetch['firstWord']) ? htmlspecialchars($fetch['firstWord']) : '';  // Check if firstWord exists
        $details = isset($fetch['details']) ? htmlspecialchars($fetch['details']) : '';  // Check if details exists

        // Display the row
        echo '- <font class="medTitle"><b><a href="possible_sides.php?name=' . $firstWord . '" style="text-decoration: none;color: inherit;">' . $medname . '</a></b></font>, ' . $timeonly . ' (' . $daystohrs . ' ' . $time_spent . ' ago)' . $details . ' <br>';
    }
