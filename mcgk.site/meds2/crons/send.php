...
$showData = $con->query("SELECT * from medlist WHERE sent_email IS NULL and default_half_life !='' AND lastdose != '' ORDER BY id DESC");
while($fetch = $showData->fetch()){

    $doseDate = $fetch['lastdose'];
    $halflife = round($fetch['default_half_life']*60*60);
    $fiveHalf = $halflife*5;
    $name = $fetch['name'];
    $id = $fetch['id'];
    ...
    // Half Life Email
    if($hours > $fetch['default_half_life']){
        ...
        $subject2 = ''.$name . ' passed half life!';
        // Send the email
        if(mail($to, $subject, $message, $headers)){
            $update = $con->query("UPDATE medlist set sent_email='1', fivehalf_email=null where id='$id'");
            $notification = $connect->query("INSERT INTO notifications (message, date_time, notif_type, title, page) VALUES ('$subject2', '$datenow', 'Meds2', '$name', 'index.php?searchKey')");
        } else {
...
$showData2 = $con->query("SELECT * from medlist WHERE fivehalf_email IS NULL and default_half_life !='' order by id desc");
while($fetch2 = $showData2->fetch()){

    $doseDate = $fetch2['lastdose'];
    $halflife = round($fetch2['default_half_life']*60*60);
    $fiveHalf = $halflife*5;
    $name = $fetch2['name'];
    $id = $fetch2['id'];
    ...
    $five_half = $fetch2['default_half_life']*5;
    if($hours > $five_half){
        $dateTime = DateTime::createFromFormat('d M, Y h:i a', $doseDate);
