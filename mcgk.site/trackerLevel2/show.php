...
if (isset($_GET['id'])) {
    ...
    $avgDay = $con->query("SELECT COUNT(start_date) as totDay,STR_TO_DATE(start_date, '%d %M, %Y') as strdate,id FROM details where activity_id='$getid' group by strdate");
...
<br><br><br><div style="margin-bottom:20px;"><span id="<?php echo $row['id']; ?>" name="<?php echo $row['start_date']; ?>" style="margin-left:10px;margin-top:80px;margin-bottom:80px;"><button id="stopbutton" class="btn btn-danger">Stop</button><i>Working...</i> <?php echo '<font size="4"> <img src="img/left_arrow.png"><span style="border:2px black solid;margin:5px;padding:3px;"><b id="timerBox">' . gmdate("H:i:s", TimerNow($row2['start_date'])) . '</b></font></span>'; ?> <img src="img/right_arrow.png"> <a href="<?php echo $actual_link; ?>" class="btn btn-info">Refresh</a><div style="display:inline;padding-left: 20px;"><img src="img/note.png"> <input type="text" id="notesinput" placeholder="Enter notes"> <button id="pasteButton">Paste</button> <button id="enterButton">Submit</button> </div></div>
 </span>
...
if ($details->rowCount() > 0) {
    ...
    while ($rows = $details->fetch()) {
        ...
        if ($rows['current_status'] == 'on') {
            ...
            $arr[$day][] = '<tr><td><div style="padding:10px;"><img src="img/dot.png"> From ' . $time . ' To ' . $timeEnd . ' (' . ConvertSeconds($rows['total_time']) . ')' . $approxIU . $showNotes . '<br>';
    ...
    foreach ($arr as $key => $value) {
        ...
        echo '<tr><th><div class="imgCalendar"><img src="img/calendar.png"><div>' . $zIndex . '</div></div><font size="5" style="color:#00f3ff;font-weight: 650;vertical-align:middle;"> ' . $key . ':</font>' . $TimeConv . '</th></tr>';
